...
def main():
    project_id = os.getenv('PROJECT_ID', "pragmatic-aegis-436316-m4")  # Substitua pelo seu ID do projeto
    location = os.getenv('LOCATION', "global")  # Pode ser 'us-central1', 'europe-west1', etc.
    
    # Solicite ao usuário o nome do arquivo de saída
    output_file_name = input("Digite o nome do arquivo de saída (ex: output.wav): ")
    output_gcs_uri = f"gs://b_txt_speech/{output_file_name}"  # Substitua pelo seu bucket e nome do arquivo

    # Solicite ao usuário o caminho do arquivo SSML
    ssml_file_path = input("Digite o caminho do arquivo SSML a ser lido: ")
    
    # Lê o texto SSML do arquivo
    ssml_text = read_ssml_from_file(ssml_file_path)
    if ssml_text is None:
        logger.error("Failed to read SSML text. Aborting process.")
        return

    # Cria o cliente TTS
    client = create_tts_client()
    
    # Chame a função para sintetizar o áudio longo
    synthesize_long_audio(client, project_id, location, output_gcs_uri, ssml_text)
