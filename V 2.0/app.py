...
def format_text():
    try:
        ...
        # Prompt específico para melhorar a pontuação e legibilidade
        prompt = f"""
        Por favor, formate o seguinte texto transcrito de áudio para melhorar sua legibilidade:
        1. Adicione pontuação adequada (vírgulas, pontos, etc.)
        2. Corrija a capitalização das palavras
        ...
        formatted_text = response.json()['candidates'][0]['content']['parts'][0]['text']
        return jsonify({'formatted_text': formatted_text})

    except requests.exceptions.RequestException as e:
        return jsonify({'error': f'Erro na API Gemini: {str(e)}'}), 500
    except Exception as e:
        return jsonify({'error': f'Erro ao formatar texto: {str(e)}'}), 500
