...
def chat_send():
    ...
    try:
        ...
        model = request.form.get('model', 'gemini-1.5-pro')
        temperature = float(request.form.get('temperature', 0.7))
        top_k = int(request.form.get('topK', 40))
        top_p = float(request.form.get('topP', 0.95))
        max_tokens = int(request.form.get('maxTokens', 2048))
        
        # Log dos parâmetros recebidos
        logger.info(f"Parâmetros recebidos na requisição:")
        ...
        if not (0 <= temperature <= 1.9):
            return jsonify({'error': 'Temperatura deve estar entre 0 e 1.9'}), 400
        if not (1 <= top_k <= 100):
            return jsonify({'error': 'Top K deve estar entre 1 e 100'}), 400
        if not (0 <= top_p <= 1):
            return jsonify({'error': 'Top P deve estar entre 0 e 1'}), 400
        if not (10 <= max_tokens <= 8192):
            return jsonify({'error': 'Max Tokens deve estar entre 10 e 8192'}), 400
        ...
        try:
            # Envia a mensagem com todos os parâmetros
            response = chat_module.send_chat_message(
                api_key=api_key,
                message=message,
