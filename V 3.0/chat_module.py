...
def prepare_chat_request(message, model=GeminiModel.GEMINI_PRO.value, image_data=None, temperature=0.7):
...
def send_chat_message(api_key, message, image_path=None, temperature=0.7):
    ...
    try:
        ...
        if image_path:
            ...
            if not image_data:
                raise ValueError("Falha no processamento da imagem")
        ...
        data = prepare_chat_request(message, model, image_data, temperature)

        # Faz a requisição
        logger.info(f"Enviando mensagem para o modelo {model}")
